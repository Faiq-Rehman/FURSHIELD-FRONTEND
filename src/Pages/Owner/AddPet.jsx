...
export default function AddPet() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to add pet. Please check fields.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="rounded-3xl bg-white p-7 shadow-sm border border-slate-100">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="font-display text-2xl font-bold text-slate-900">Add a New Pet</h1>
            <p className="text-xs text-slate-500">Enter details to track health, visits, and care history.</p>
          </div>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="pt-4 flex gap-3">
            ...
            >
              {submitting ? 'Saving Pet...' : 'Save Pet Profile'}
            </button>
            <Link
              to="/owner/pets"
